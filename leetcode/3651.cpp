...
public:
    int minCost(vector<vector<int>>& grid, int k) {
        ...
        for (int t = 0; t <= k; t++) {
            ...
            // LeetCode 64: Minimum Path Sum (space-optimized DP)
            ranges::fill(f, INT_MAX / 2);
            // Do not count the starting cell cost
            f[1] = -grid[0][0];
            ...
            // Converged: teleportation no longer changes suf_min_f,
            // so further teleport operations will have no effect
            if (suf_min_f == tmp) {
                break;
            }
        }
